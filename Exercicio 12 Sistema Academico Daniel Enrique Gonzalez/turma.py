...
class LimiteConsultaTurmas():
    def __init__(self,str):
        messagebox.showinfo('Turmas Consultadas',str)
...
class CtrlTurma():       
    ...
    def consultaTurmas(self):
        numM = simpledialog.askstring("Turmas","Qual é o código da disciplina?")
        str = ''
        s = 0
        for n in self.listaTurmas:
          if n.disciplina.codigo == numM:
            s = 1
            str += 'Código: ' + n.codigo + '\n'
            str += 'Disciplina: ' + n.disciplina.codigo + '\n'
            str += 'Estudantes:\n'
            for estud in n.estudantes:
                str += estud.nroMatric + ' - ' + estud.nome + '\n'
            str += '------\n'
        if s == 0:
            str = 'Não existem turmas com esta disciplina'
        if numM != None:
          LimiteConsultaTurmas(str)